...
LABELS = {
    "vanilla": "Linux Vanilla",
    "shared": "SPARe PT shared",
    "diverged": "SPARe PT diverged",
}
COLORS = {
...
def draw(ax, df: pd.DataFrame, bars, ylabel: str = "µs", tick_fs=6):
    ...
    once per node count. Ticks name the bar."""
...
def legend_handles() -> list[Patch]:
    return [
        ...
        for state in ("vanilla", "shared", "diverged")
    ]
...
def make_legend():
    ...
    fig.legend(
        handles=legend_handles(), loc="center", ncol=3, fontsize=7, frameon=False
    )
...
def plot_chunks(arch: str, sub: str):
    ...
    handles = [
        plt.Rectangle((0, 0), 1, 1, color=COLORS[s])
        for s, _, _ in CHUNK_STATES
    ]
    fig.legend(
        handles,
        [LABELS[s] for s, _, _ in CHUNK_STATES],
