...
VALUE_TAG = "patched-repl"  # the bar the QPS numbers sit on
TAGS_ORDER = [
    # "imbalanced-memory",
    "default",
    "interleaved-memory",
...
palettes = {
    ...
    "default": linux[0],  # the pale orange llama gives First Touch
...
YLABEL_SIZE = 5.5
TITLE_SIZE = 6.5
TITLE_PAD = 18  # points: just clears the stood-up values above the bars
XTICK_SIZE = 5.5
XTICK_PAD = 0.5  # points between the dataset labels and the x axis
VALUE_SIZE = 4.5  # QPS printed over each bar, stood up
BAR_WIDTH = 0.095
BAR_GAP = 0.0
X_SPACING = 0.48
...
# the pct panels hold one bar group per dataset, so they sit lower than
# the absolute ones
PCT_HEIGHT = 0.8
ABS_HEIGHT = 1.0


def _make_subplots(sharey: bool, wspace: float, height: float = ABS_HEIGHT):
...
def _format_runner_ax(
    ...
    # a stood-up value can reach past the panel, and the next panel's own
    # white background would paint over its tail
...
def _short_value(value: float) -> str:
    """Thousands with one decimal and a small k, with the unit: "21.4k Q/s"."""
    return f"{value / 1000:.1f}k Q/s" if value >= 1000 else f"{value:.0f} Q/s"
...
def plot_main(df: pd.DataFrame):
    ...
    for arch in df["arch"].unique():
        ...
        fig, axes = _make_subplots(
            sharey=True, wspace=0.05, height=PCT_HEIGHT
        )
        ...
        for idx, runner in enumerate(RUNNER_NAMES):
            ...
            for i, tag in enumerate(TAGS_ORDER):
                ...
                # one QPS per group, off the SPARe bar: the others are
...
def plot_main_abs(df: pd.DataFrame, df_norm: pd.DataFrame):
    """Plot absolute throughput with percentage improvement over Linux Vanilla.
